...
class Robot
  ...
  # Returns true if the robot is already placed on the table, returns false othersise. The Robot 
  # is placed on the table when receving a valid PLACE command. When not on the table it will ignore 
  # other commands such as MOVE, LEFT, RIGHT. A robot is only on table when the x, y and facing 
  # has value
  def on_the_table?
    !!(x && y && facing)
  end
  ...
  # Returns true if it's placed validly, otherwise returns false
